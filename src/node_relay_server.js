...
class NodeRelayServer {
  ...
  async run() {
    try {
      ...
      Logger.error(
        `Node Media Relay Server startup failed. ffmpeg:${this.config.relay.ffmpeg} cannot be executed.`
      )
      return
    }
    ...
    if (version === "" || parseInt(version.split(".")[0]) < 4) {
      Logger.error(
        "Node Media Relay Server startup failed. ffmpeg requires version 4.0.0 above"
      )
  ...
  onRelayPull(url, app, name) {
    ...
    session.run()
    Logger.log(
      "[relay dynamic pull] start id=" + id,
      conf.inPath,
      "to",
      conf.ouPath
    )
  ...
  onRelayPush(url, app, name) {
    ...
    session.run()
    Logger.log(
      "[relay dynamic push] start id=" + id,
      conf.inPath,
      "to",
      conf.ouPath
    )
