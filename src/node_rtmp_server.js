...
class NodeRtmpServer {
  constructor(config) {
    config.rtmp.port = this.port = config.rtmp.port
      ? config.rtmp.port
      : RTMP_PORT
    ...
    if (config.rtmp.ssl) {
      config.rtmp.ssl.port = this.sslPort = config.rtmp.ssl.port
        ? config.rtmp.ssl.port
        : RTMPS_PORT
      try {
        const options = {
        ...
        Logger.error(
          `Node Media Rtmps Server error while reading ssl certs: <${e}>`
        )
      }
    }
  }
  ...
  stop() {
    ...
    context.sessions.forEach((session, id) => {
      if (session instanceof NodeRtmpSession) session.stop()
    })
  }
}
