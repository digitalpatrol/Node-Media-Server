...
class NodeTransServer {
  ...
  async run() {
    ...
    if (version === "" || parseInt(version.split(".")[0]) < 4) {
      Logger.error(
        "Node Media Trans Server startup failed. ffmpeg requires version 4.0.0 above"
      )
    ...
    Logger.log(
      `Node Media Trans Server started for apps: [ ${apps}] , MediaRoot: ${this.config.http.mediaroot}, ffmpeg version: ${version}`
    )
  }
